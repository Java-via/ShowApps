...
SQL_YS_SPEED_TOP_SOGTGAME = """SELECT a_getdate, a_picurl, a_pkgname, a_name, TRUNCATE(a_rateinstall*100,2) FROM t_apps_speed
...
SQL_CRNT_SPEED_TOP_SOFTGAME = """SELECT t1.a_pkgname, t1.a_name, t1.a_getdate, CAST((t1.a_install-t2.a_install)/t2.a_install AS CHAR) AS a_subinstall
                                  ...
                                  (
                                    SELECT a_pkgname
                                    FROM t_apps_speed
                                    WHERE a_softgame = %s AND a_isspeed = 1 AND
